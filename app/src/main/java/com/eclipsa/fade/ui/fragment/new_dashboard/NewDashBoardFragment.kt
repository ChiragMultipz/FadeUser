...
import com.eclipsa.fade.databinding.FragmentNewDashBoardBinding
...
import com.eclipsa.fade.ui.change_location.ChangeLocationActivity
import com.eclipsa.fade.ui.notification.NotificationActivity
...
class NewDashBoardFragment(otherAddressLatLon: String) :
                    ...
                    if (it.value.result != null) {
                        ...
                        if (it.value.result.nearByBarbers != null) {
                            for (i in it.value.result.nearByBarbers.indices) {
                                ...
                                serviceListFilter =
                                    it.value.result.nearByBarbers[i]!!.services!!.sortedBy { it!!.service_price } as List<ServicesItem>
    ...
    private fun searchData(searchKeyword: String) {
        ...
        serviceList.forEach {
            if (it.name!!.lowercase(Locale.getDefault()).contains(searchKeyword.lowercase(Locale.getDefault()))) {
                filteredServiceItem.add(it)
            }
        }

        for (barber in nearByBarberList) {
            if (barber.name!!.lowercase(Locale.getDefault()).contains(searchKeyword.lowercase(Locale.getDefault()))) {
                newBarberList.add(barber)
            }
            for (service in barber.services!!) {
                if (service!!.service!!.name!!.lowercase(Locale.getDefault())
    ...
    private fun filterBarberData(item: FilterModel) {
        when (item.filter) {
            ...
            Constants.FILTER_RATING -> {
                if (tempNearByBarberList.isNotEmpty()) {
                    tempNearByBarberList =
                        tempNearByBarberList.sortedByDescending { it.average_rating }
                    nearByBarberAdapter.updateList(tempNearByBarberList)
                } else if (servicesAdapter.getSelectedServices().isEmpty()) {
